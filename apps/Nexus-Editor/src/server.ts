...
export async function createServer() {
  ...
  const baseUrl =
    process.env.NEXUS_EDITOR_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new EditorEngine("data/nexus-editor.sqlite");

  const phantom = new PhantomApp("nexus-editor");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled:
                (process.env["NEXUS_EDITOR_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
              cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787",
            },
            phantom: phantom.status(),
          },
      ...
      if (req.method === "GET" && p === "/api/v1/editor/editors")
        return json(engine.listEditors());

      if (req.method === "POST" && p === "/api/v1/editor/editors") {
        const b = (await req.json().catch(() => ({}))) as any;
        if (!b.name) return json({ error: "name required" }, 400);
        return json(engine.createEditor(b.name, b.type), 201);
      }

      const gm = p.match(/^\/api\/v1\/editor\/editors\/([^/]+)$/);
      if (req.method === "GET" && gm) {
        const entity = engine.getEditor(gm[1]!);
  ...
  console.log(`[nexus-editor] Listening on port ${server.port}`);
