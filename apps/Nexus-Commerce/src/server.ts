...
export async function createServer() {
  ...
  const baseUrl =
    process.env.NEXUS_COMMERCE_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new CommerceEngine("data/nexus-commerce.sqlite");

  const phantom = new PhantomApp("nexus-commerce");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled:
                (process.env["NEXUS_COMMERCE_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
              cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787",
            },
            phantom: phantom.status(),
          },
      ...
      if (req.method === "GET" && p === "/api/v1/commerce/commerces")
        return json(engine.listCommerces());

      if (req.method === "POST" && p === "/api/v1/commerce/commerces") {
        const b = (await req.json().catch(() => ({}))) as any;
        if (!b.name) return json({ error: "name required" }, 400);
        return json(engine.createCommerce(b.name, b.type), 201);
      }

      const gm = p.match(/^\/api\/v1\/commerce\/commerces\/([^/]+)$/);
      if (req.method === "GET" && gm) {
        const entity = engine.getCommerce(gm[1]!);
  ...
  console.log(`[nexus-commerce] Listening on port ${server.port}`);
