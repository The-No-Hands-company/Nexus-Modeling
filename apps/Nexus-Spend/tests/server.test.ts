...
describe("nexus-spend", () => { let base = ""; let handle: Awaited<ReturnType<typeof createServer>>;
  ...
  it("POST /api/v1/spend/budgets creates budget", async () => { const res = await fetch(`${base}/api/v1/spend/budgets`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: "Monthly Food", category: "Food", amount: 500, period: "2026-06" }) }); expect(res.status).toBe(201); const body = await res.json() as any; expect(body.name).toBe("Monthly Food"); });
  it("GET /api/v1/spend/budgets lists budgets", async () => { const res = await fetch(`${base}/api/v1/spend/budgets`); expect(res.status).toBe(200); const body = await res.json() as any[]; expect(Array.isArray(body)).toBe(true); });
});
