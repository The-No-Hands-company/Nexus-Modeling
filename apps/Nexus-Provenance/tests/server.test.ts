...
describe("nexus-provenance", () => { let base = ""; let handle: Awaited<ReturnType<typeof createServer>>;
  ...
  it("GET /api/v1/provenance/records lists records", async () => { const res = await fetch(`${base}/api/v1/provenance/records`); expect(res.status).toBe(200); const body = await res.json() as any[]; expect(Array.isArray(body)).toBe(true); });
  it("GET /api/v1/provenance/chains lists chains", async () => { const res = await fetch(`${base}/api/v1/provenance/chains`); expect(res.status).toBe(200); const body = await res.json() as any[]; expect(Array.isArray(body)).toBe(true); });
});
