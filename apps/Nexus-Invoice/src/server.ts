...
export async function createServer() {
  ...
  const baseUrl =
    process.env.NEXUS_INVOICE_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new InvoiceEngine("data/nexus-invoice.sqlite");

  const phantom = new PhantomApp("nexus-invoice");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled:
                (process.env["NEXUS_INVOICE_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
              cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787",
            },
            phantom: phantom.status(),
          },
      ...
      if (req.method === "GET" && p === "/api/v1/invoice/invoices")
        return json(engine.listInvoices());

      if (req.method === "POST" && p === "/api/v1/invoice/invoices") {
        const b = (await req.json().catch(() => ({}))) as any;
        if (!b.name) return json({ error: "name required" }, 400);
        return json(engine.createInvoice(b.name, b.amount), 201);
      }

      const gm = p.match(/^\/api\/v1\/invoice\/invoices\/([^/]+)$/);
      if (req.method === "GET" && gm) {
        const entity = engine.getInvoice(gm[1]!);
  ...
  console.log(`[nexus-invoice] Listening on port ${server.port}`);
