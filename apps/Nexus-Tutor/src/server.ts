...
export function createServer() {
  ...
  const baseUrl = process.env.NEXUS_NEXUS_TUTOR_BASE_URL || `http://localhost:${port}`;
    ...
    async fetch(request) {
      ...
      if (request.method === "GET" && path === "/api/v1/status") {
        return json({
          service: "nexus-tutor",
          status: "ready",
          capabilities: ["tutor", "ai-education", "adaptive-learning"],
          cloudIntegration: {
            enabled: (process.env["NEXUS_TUTOR_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
  ...
  console.log(`[nexus-tutor] Listening on port ${server.port}`);
