...
function hbMs(): number {
  return Math.max(
    5000,
    Number(
      process.env["NEXUS_KNOWLEDGE_CLOUD_HEARTBEAT_INTERVAL_MS"] || "30000",
    ),
  );
}
...
function enabled(): boolean {
  return (
    (process.env["NEXUS_KNOWLEDGE_ENABLE_CLOUD_INTEGRATION"] || "true")
      .trim()
      .toLowerCase() !== "false"
  );
}
...
export async function registerWithCloud(baseUrl: string): Promise<void> {
  ...
  if (!r.ok) throw new Error(`Nexus-Knowledge registration failed: ${r.status}`);
}

export async function heartbeatWithCloud(baseUrl: string): Promise<void> {
  const r = await fetch(
    `${cloudBaseUrl()}/api/v1/tools/${encodeURIComponent("nexus-knowledge")}/heartbeat`,
    {
      method: "POST",
      headers: cloudHeaders(),
  ...
  if (!r.ok)
    throw new Error(`Nexus-Knowledge heartbeat failed: ${r.status}`);
}
...
export function startHeartbeat(baseUrl: string): () => void {
  ...
  registerWithCloud(baseUrl).catch((e) => {
    console.warn(
      `[nexus-knowledge] Cloud registration failed: ${(e as Error).message}`,
    );
  });
  ...
  const timer = setInterval(() => {
    heartbeatWithCloud(baseUrl).catch((e) => {
      console.warn(
        `[nexus-knowledge] Cloud heartbeat failed: ${(e as Error).message}`,
      );
    });
  }, hbMs());
