...
export async function createServer() {
  ...
  const phantom = new PhantomApp("nexus-design");
...
;
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (req.method === "GET" && p === "/api/v1/status") return json({ service: "nexus-design", status: "ready", capabilities: ["design", "prototyping", "collaboration"], cloudIntegration: { enabled: (process.env["NEXUS_DESIGN_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false", cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787" }, phantom: phantom.status() }, 200);
