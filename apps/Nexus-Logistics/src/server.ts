...
export async function createServer() {
  ...
  const baseUrl =
    process.env.NEXUS_LOGISTICS_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new LogisticsEngine("data/nexus-logistics.sqlite");

  const phantom = new PhantomApp("nexus-logistics");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled:
                (process.env["NEXUS_LOGISTICS_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
              cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787",
            },
            phantom: phantom.status(),
          },
      ...
      if (req.method === "GET" && p === "/api/v1/logistics/logisticses")
        return json(engine.listLogisticss());

      if (req.method === "POST" && p === "/api/v1/logistics/logisticses") {
        const b = (await req.json().catch(() => ({}))) as any;
        if (!b.name) return json({ error: "name required" }, 400);
        return json(engine.createLogistics(b.name, b.type), 201);
      }

      const gm = p.match(/^\/api\/v1\/logistics\/logisticses\/([^/]+)$/);
      if (req.method === "GET" && gm) {
        const entity = engine.getLogistics(gm[1]!);
  ...
  console.log(`[nexus-logistics] Listening on port ${server.port}`);
