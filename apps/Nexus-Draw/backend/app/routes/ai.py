...
from app.tasks import generate_diagram_task, remove_background_task

router = APIRouter(prefix="/api/v1/draw/ai", tags=["ai"])
...
async def generate_diagram(body: GenerateRequest):
    ...
    task = generate_diagram_task.delay(body.prompt, body.width, body.height, body.style, body.board_id)
