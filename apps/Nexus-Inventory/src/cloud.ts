...
function hbMs(): number {
  return Math.max(
    5000,
    Number(
      process.env["NEXUS_INVENTORY_CLOUD_HEARTBEAT_INTERVAL_MS"] || "30000",
    ),
  );
}
...
function enabled(): boolean {
  return (
    (process.env["NEXUS_INVENTORY_ENABLE_CLOUD_INTEGRATION"] || "true")
      .trim()
      .toLowerCase() !== "false"
  );
}
...
export async function registerWithCloud(baseUrl: string): Promise<void> {
  ...
  if (!r.ok) throw new Error(`Nexus-Inventory registration failed: ${r.status}`);
}

export async function heartbeatWithCloud(baseUrl: string): Promise<void> {
  const r = await fetch(
    `${cloudBaseUrl()}/api/v1/tools/${encodeURIComponent("nexus-inventory")}/heartbeat`,
    {
      method: "POST",
      headers: cloudHeaders(),
  ...
  if (!r.ok)
    throw new Error(`Nexus-Inventory heartbeat failed: ${r.status}`);
}
...
export function startHeartbeat(baseUrl: string): () => void {
  ...
  registerWithCloud(baseUrl).catch((e) => {
    console.warn(
      `[nexus-inventory] Cloud registration failed: ${(e as Error).message}`,
    );
  });
  ...
  const timer = setInterval(() => {
    heartbeatWithCloud(baseUrl).catch((e) => {
      console.warn(
        `[nexus-inventory] Cloud heartbeat failed: ${(e as Error).message}`,
      );
    });
  }, hbMs());
