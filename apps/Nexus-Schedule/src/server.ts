...
export async function createServer() {
  ...
  const baseUrl =
    process.env.NEXUS_SCHEDULE_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new ScheduleEngine("data/nexus-schedule.sqlite");

  const phantom = new PhantomApp("nexus-schedule");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled:
                (process.env["NEXUS_SCHEDULE_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
              cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787",
            },
            phantom: phantom.status(),
          },
      ...
      if (req.method === "GET" && p === "/api/v1/schedule/schedules")
        return json(engine.listSchedules());

      if (req.method === "POST" && p === "/api/v1/schedule/schedules") {
        const b = (await req.json().catch(() => ({}))) as any;
        if (!b.name) return json({ error: "name required" }, 400);
        return json(engine.createSchedule(b.name, b.type), 201);
      }

      const gm = p.match(/^\/api\/v1\/schedule\/schedules\/([^/]+)$/);
      if (req.method === "GET" && gm) {
        const entity = engine.getSchedule(gm[1]!);
  ...
  console.log(`[nexus-schedule] Listening on port ${server.port}`);
