...
export async function createServer() {
  ...
  const baseUrl =
    process.env.NEXUS_PUBLISHING_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new PublishingEngine("data/nexus-publishing.sqlite");

  const phantom = new PhantomApp("nexus-publishing");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled:
                (process.env["NEXUS_PUBLISHING_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
              cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787",
            },
            phantom: phantom.status(),
          },
      ...
      if (req.method === "GET" && p === "/api/v1/publishing/publishings")
        return json(engine.listPublishings());

      if (req.method === "POST" && p === "/api/v1/publishing/publishings") {
        const b = (await req.json().catch(() => ({}))) as any;
        if (!b.name) return json({ error: "name required" }, 400);
        return json(engine.createPublishing(b.name, b.type), 201);
      }

      const gm = p.match(/^\/api\/v1\/publishing\/publishings\/([^/]+)$/);
      if (req.method === "GET" && gm) {
        const entity = engine.getPublishing(gm[1]!);
  ...
  console.log(`[nexus-publishing] Listening on port ${server.port}`);
