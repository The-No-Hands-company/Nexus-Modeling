...
export async function createServer() {
  ...
  const baseUrl =
    process.env.NEXUS_JOURNAL_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new JournalEngine("data/nexus-journal.sqlite");

  const phantom = new PhantomApp("nexus-journal");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled:
                (process.env["NEXUS_JOURNAL_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
              cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787",
            },
            phantom: phantom.status(),
          },
      ...
      if (req.method === "GET" && p === "/api/v1/journal/journals")
        return json(engine.listJournals());

      if (req.method === "POST" && p === "/api/v1/journal/journals") {
        const b = (await req.json().catch(() => ({}))) as any;
        if (!b.name) return json({ error: "name required" }, 400);
        return json(engine.createJournal(b.name, b.type), 201);
      }

      const gm = p.match(/^\/api\/v1\/journal\/journals\/([^/]+)$/);
      if (req.method === "GET" && gm) {
        const entity = engine.getJournal(gm[1]!);
  ...
  console.log(`[nexus-journal] Listening on port ${server.port}`);
