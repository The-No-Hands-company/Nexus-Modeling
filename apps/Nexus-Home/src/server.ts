...
export async function createServer() {
  ...
  const phantom = new PhantomApp("nexus-home");
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (request.method === "GET" && path === "/api/v1/status") {
        return json({ service: "nexus-home", status: "ready", capabilities: ["smart-home", "iot", "automation"], cloudIntegration: { enabled: (process.env["NEXUS_HOME_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false", cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787" }, phantom: phantom.status() }, 200);
      }
