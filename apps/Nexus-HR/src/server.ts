...
export async function createServer() {
  ...
  const phantom = new PhantomApp("nexus-hr");
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (request.method === "GET" && path === "/api/v1/status") {
        return json({ service: "nexus-hr", status: "ready", capabilities: ["hr", "onboarding", "payroll"], cloudIntegration: { enabled: (process.env["NEXUS_HR_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false", cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787" }, phantom: phantom.status() }, 200);
      }
