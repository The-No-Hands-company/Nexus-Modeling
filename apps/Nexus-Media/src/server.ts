  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (request.method === "GET" && path === "/api/v1/status") {
        return json({ service: "nexus-media", status: "ready", capabilities: ["media", "streaming", "library"], cloudIntegration: { enabled: (process.env["NEXUS_MEDIA_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false", cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787" } }, 200);
      }
