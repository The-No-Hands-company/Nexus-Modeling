...
import { BrowsingEngine } from "./browsing-engine";
...
export async function createServer() {
  ...
  const baseUrl = process.env.NEXUS_NEXUS_BROWSING_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new BrowsingEngine("data/nexus-browsing.sqlite")
  const phantom = new PhantomApp("nexus-browsing");
...
;
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled: (process.env["NEXUS_BROWSING_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
      ...
      if (req.method === "POST" && p === "/api/v1/items") {
        ...
        return json(engine.create(b.name, b.bookmarks, b.history, b.collections), 201);
      }
  ...
  console.log(`[nexus-browsing] Listening on port ${server.port}`);
