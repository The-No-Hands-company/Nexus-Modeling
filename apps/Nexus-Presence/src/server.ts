...
export function createServer() {
  ...
  const baseUrl = process.env.NEXUS_NEXUS_PRESENCE_BASE_URL || `http://localhost:${port}`;
    ...
    async fetch(request) {
      ...
      if (request.method === "GET" && path === "/api/v1/status") {
        return json({
          ...
          cloudIntegration: {
            enabled: (process.env["NEXUS_PRESENCE_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
  ...
  console.log(`[nexus-presence] Listening on port ${server.port}`);
