  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (request.method === "GET" && path === "/api/v1/status") {
        return json({ service: "nexus-market", status: "ready", capabilities: ["marketplace", "trading", "federation"], cloudIntegration: { enabled: (process.env["NEXUS_MARKET_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false", cloudUrl: process.env.NEXUS_CLOUD_URL || "http://localhost:8787" } }, 200);
      }
