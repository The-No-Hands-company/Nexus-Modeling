...
TEST(SceneAsset, PackageMigrationChainUpgradesOldVersion)
{
    const std::string path = tmpPath("pkg_manifest_migration_chain.nxpk");

    // Build a legacy v0 package blob directly.
    std::FILE* fp = std::fopen(path.c_str(), "wb");
    ASSERT_NE(fp, nullptr);
    ...
    ASSERT_TRUE(writeU32(0u)); // legacy v0
    ASSERT_TRUE(writeU32(2u));

    // v0 entry layout: [path][dependsCount][depends...]
    ASSERT_TRUE(writeString("/tmp/mig_a.nxas"));
    ASSERT_TRUE(writeU32(0u));
    ASSERT_TRUE(writeString("/tmp/mig_b.nxas"));
    ASSERT_TRUE(writeU32(1u));
    ASSERT_TRUE(writeString("/tmp/mig_a.nxas"));
    std::fclose(fp);

    SceneAsset::resetBuiltinPackageMigrations();

    SceneAssetPackageDescriptor loaded;
    const SceneAssetPackageIOReport rep = SceneAsset::loadPackageManifest(path, loaded);
    ASSERT_TRUE(rep.valid);
    EXPECT_EQ(rep.version, kSceneAssetPackageVersionCurrent);
    ASSERT_EQ(loaded.entries.size(), 2u);
    EXPECT_EQ(loaded.entries[0].name, loaded.entries[0].path);
    EXPECT_EQ(loaded.entries[1].name, loaded.entries[1].path);

    std::remove(path.c_str());
}

TEST(SceneAsset, PackageMigrationFailureReturnsError)
{
    const std::string path = tmpPath("pkg_manifest_migration_fail.nxpk");

    std::FILE* fp = std::fopen(path.c_str(), "wb");
    ASSERT_NE(fp, nullptr);
    const uint32_t magic = kSceneAssetPackageMagic;
    const uint32_t version = 0u;
    const uint32_t count = 0u;
    ASSERT_EQ(std::fwrite(&magic, sizeof(magic), 1, fp), 1u);
    ASSERT_EQ(std::fwrite(&version, sizeof(version), 1, fp), 1u);
    ASSERT_EQ(std::fwrite(&count, sizeof(count), 1, fp), 1u);
    std::fclose(fp);

    ASSERT_TRUE(SceneAsset::registerPackageMigration(
        0u,
        [](uint32_t, std::vector<uint8_t>&) {
            return false;
        }));
    ...
    const SceneAssetPackageIOReport rep = SceneAsset::loadPackageManifest(path, loaded);
    EXPECT_FALSE(rep.valid);
    EXPECT_TRUE(hasPackageDiagnostic(rep.diagnostic, SceneAssetPackageDiagnostic::MigrationFailed));

    SceneAsset::resetBuiltinPackageMigrations();
    std::remove(path.c_str());
}

TEST(SceneAsset, PackageManifestPersistsMigrationPolicyMetadata)
{
    const std::string path = tmpPath("pkg_manifest_policy_metadata.nxpk");

    SceneAssetPackageDescriptor pkg;
    pkg.migrationPolicy.targetVersion = 2u;
    pkg.migrationPolicy.modeFlags = SceneAssetPackageMigrationModeFlags::Lenient;
    pkg.entries = {
        SceneAssetPackageEntry{"/tmp/meta_a.nxas", "A", {}},
    };

    const SceneAssetPackageIOReport saveRep = SceneAsset::savePackageManifest(pkg, path);
    ASSERT_TRUE(saveRep.valid);
    EXPECT_EQ(saveRep.targetVersion, 2u);
    EXPECT_TRUE(hasMigrationModeFlag(saveRep.modeFlags,
                                     SceneAssetPackageMigrationModeFlags::Lenient));

    SceneAssetPackageDescriptor loaded;
    const SceneAssetPackageIOReport loadRep = SceneAsset::loadPackageManifest(path, loaded);
    ASSERT_TRUE(loadRep.valid);
    EXPECT_EQ(loaded.migrationPolicy.targetVersion, 2u);
    EXPECT_TRUE(hasMigrationModeFlag(loaded.migrationPolicy.modeFlags,
                                     SceneAssetPackageMigrationModeFlags::Lenient));

    std::remove(path.c_str());
}

TEST(SceneAsset, PackageManifestStrictModeRejectsSourceAboveTarget)
{
    const std::string path = tmpPath("pkg_manifest_strict_target.nxpk");

    SceneAssetPackageDescriptor pkg;
    pkg.entries = {SceneAssetPackageEntry{"/tmp/strict_a.nxas", "A", {}}};
    ASSERT_TRUE(SceneAsset::savePackageManifest(pkg, path).valid);

    SceneAssetPackageDescriptor loaded;
    SceneAssetPackageMigrationPolicy policy{};
    policy.targetVersion = 1u;
    policy.modeFlags = SceneAssetPackageMigrationModeFlags::Strict;

    const SceneAssetPackageIOReport rep =
        SceneAsset::loadPackageManifest(path, loaded, policy);
    EXPECT_FALSE(rep.valid);
    EXPECT_TRUE(hasPackageDiagnostic(rep.diagnostic,
                                     SceneAssetPackageDiagnostic::UnsupportedVersion));

    std::remove(path.c_str());
}

TEST(SceneAsset, PackageManifestLenientModeClampsTargetAndCapturesAuditTrail)
{
    const std::string path = tmpPath("pkg_manifest_lenient_audit.nxpk");

    // Write a legacy v1 package blob: [magic][version][entryCount][entries...]
    std::FILE* fp = std::fopen(path.c_str(), "wb");
    ASSERT_NE(fp, nullptr);
    const auto writeU32 = [&](uint32_t v) {
        return std::fwrite(&v, sizeof(v), 1, fp) == 1u;
    ...
    ASSERT_TRUE(writeU32(1u));
    ASSERT_TRUE(writeU32(1u));
    ASSERT_TRUE(writeString("/tmp/lenient_a.nxas"));
    ASSERT_TRUE(writeString("lenient_a"));
    ASSERT_TRUE(writeU32(0u));
    std::fclose(fp);

    SceneAssetPackageDescriptor loaded;
    SceneAssetPackageMigrationPolicy policy{};
    policy.targetVersion = 999u;
    policy.modeFlags = SceneAssetPackageMigrationModeFlags::Lenient;

    const SceneAssetPackageIOReport rep =
        SceneAsset::loadPackageManifest(path, loaded, policy);
    ASSERT_TRUE(rep.valid);
    EXPECT_EQ(rep.sourceVersion, 1u);
    EXPECT_EQ(rep.version, kSceneAssetPackageVersionCurrent);
    EXPECT_FALSE(rep.migrationAuditTrail.empty());
    EXPECT_EQ(rep.migrationAuditTrail[0].fromVersion, 1u);
    EXPECT_EQ(rep.migrationAuditTrail[0].toVersion, 2u);
    EXPECT_TRUE(rep.migrationAuditTrail[0].success);
