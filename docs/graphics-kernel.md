...
| RT/mesh-shader production path (M4) | ✅ Complete | `FrameStats::activeRenderMode` reflects post-capability-downgrade mode; `setRayTracingPipeline` / `rayTracingPipeline` API; `FrameStats::rtReflectionsActive`; `traceRays` dispatch gated on HybridRT/PathTrace + RT caps + valid RT pipeline; 13 tests (Month 22) |
