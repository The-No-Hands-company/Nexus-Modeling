...
SCENARIO_ID="softrast_sphere_specular"
FILTER="SoftrastScenario.RenderSphereSpecular"
...
if [[ ! -x "${TEST_BIN}" ]]; then
    ...
    if ! SOFTRAST_SPECULAR_ARTIFACT_DIR="${ARTIFACT_DIR}" \
...
cat >"${ARTIFACT_DIR}/summary.json" <<EOF
{
  ...
  "mesh": "uv_sphere_r1_48lat_48lon",
  "shading": "gouraud+blinn_phong_specular",
  "artifact_version": 1
}
EOF
